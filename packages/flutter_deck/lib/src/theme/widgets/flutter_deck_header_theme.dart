...
class FlutterDeckHeaderThemeData {
  ...
  const FlutterDeckHeaderThemeData({this.color, this.textStyle});
  ...
  /// values.
  FlutterDeckHeaderThemeData copyWith({Color? color, TextStyle? textStyle}) {
    return FlutterDeckHeaderThemeData(color: color ?? this.color, textStyle: textStyle ?? this.textStyle);
  }
...
class FlutterDeckHeaderTheme extends InheritedTheme {
  ...
  const FlutterDeckHeaderTheme({required this.data, required super.child, super.key});
  ...
  static FlutterDeckHeaderThemeData of(BuildContext context) {
    final theme = context.dependOnInheritedWidgetOfExactType<FlutterDeckHeaderTheme>();

    return theme?.data ?? FlutterDeckTheme.of(context).headerTheme;
  }

  @override
  bool updateShouldNotify(FlutterDeckHeaderTheme oldWidget) => data != oldWidget.data;

  @override
  Widget wrap(BuildContext context, Widget child) => FlutterDeckHeaderTheme(data: data, child: child);
}
